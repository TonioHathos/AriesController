...
class TSPAgent(AriesAgent):
    ...
    def generate_credential_offer(self, aip, cred_type, cred_def_id, exchange_tracing, name, age, degree):
        ...
        birth_date = datetime.date(d.year - age)
        date_format = "%Y%m%d"
        if aip == 10:
...
async def main(args):
    ...
    try:
        ...
        await tsp_agent.generate_invitation(
            display_qr=True, reuse_connections=tsp_agent.reuse_connections, wait=True
        )
            ...
            if option is None or option in "xX":
                ...
                if tsp_agent.aip == 10:
                    offer_request = tsp_agent.agent.generate_credential_offer(
                        tsp_agent.aip, None, tsp_agent.cred_def_id, exchange_tracing
                    )
                    await tsp_agent.agent.admin_POST(
                ...
                if tsp_agent.aip == 10:
                    proof_request_web_request = (
                        tsp_agent.agent.generate_proof_request_web_request(
                            tsp_agent.aip,
                            tsp_agent.cred_type,
                            tsp_agent.revocation,
                            exchange_tracing,
                        )
                    )
                    await tsp_agent.agent.admin_POST(
                    ...
                    if tsp_agent.cred_type == CRED_FORMAT_INDY:
                        ...
                        raise Exception(
                            "Error invalid credential type:" + tsp_agent.cred_type
                        )
